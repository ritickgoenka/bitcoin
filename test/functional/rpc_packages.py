...
class RPCPackagesTest(BitcoinTestFramework):
    ...
    def test_ancestor_limits_bushy(self):
        """Create a 5-tree with 20 transactions in the mempool and 6 in the package:
        ...
        for _ in range(5):
            ...
            for _ in range(4): # Make mempool transactions M(4i+1)...M(4i+4)
                ...
                grandparents_tx.append(tx)
                grandparents_values.append(value)
                grandparents_locking_scripts.append(parent_locking_script)
                node.sendrawtransaction(txhex)
            # Package transaction Pi
            pi_hex = self.create_child_with_parents(grandparents_tx, grandparents_values, grandparents_locking_scripts)
            ...
            parent_locking_scripts.append(pi_tx.vout[0].scriptPubKey.hex())
        # Package transaction PC
        package_hex.append(self.create_child_with_parents(parent_txns, parent_values, parent_locking_scripts))
